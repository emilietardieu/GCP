...
def regrouper_dossiers_images(chemin_image , dossier_cible):
    """
    ...
    if chemin_image.exists() :
        ...
        # # Créer un dossier nommé d'après le chemin parent de l'image
        # dossier_parent = dossier_cible / chemin_image.parent.as_posix().replace("/", "_").replace("_media_nas-production", "O:").replace("_media_nas-archives", "O:")
        # dossier_parent.mkdir(parents=True, exist_ok=True)


        # # Copier avec barre de progression
        # print(f"Copie de : {chemin_image }")
        # shutil.copy2(chemin_image, dossier_parent )
        # print(f"Image copié")
